defmodule Parser do
  ...
  defp extract_module_name(module_info = %{line_number: line_number}, [
         {:__aliases__, meta, names} | _t
       ]),
       do: update_module_name(module_info, names, Keyword.get(meta, :line) == line_number)
  ...
  defp extract_functions([], module_info = %{functions: functions}),
    do: %{
      module_info
      | functions: Enum.sort(functions, fn a, b -> a.line_number < b.line_number end)
    }
